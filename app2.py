...
try:
    ee.Initialize(project='pythonmainproject')
...
def submit_coordinates():
    ...
    try:
        ...
        # Load ESA WorldCover data for time series
        areas_collection = {}
        landcover_collection_image_urls = []
        for month in range(1, 13):
            landcover_collection_image = ee.ImageCollection('ESA/WorldCover/v100') \
                .filter(ee.Filter.date(ee.Date.fromYMD(2020, 1, 1), ee.Date.fromYMD(2020, month, 28))) \
                .filterBounds(rectangle)

            landcover_collection_image = landcover_collection_image.mean()
            ...
            landcover_collection_image_edit = landcover_collection_image.clip(rectangle).visualize(**landcoverVisual)
            landcover_collection_image_url = landcover_collection_image_edit.getThumbURL({'region': rectangle, 'dimensions': 500})
            landcover_collection_image_urls.append(landcover_collection_image_url)
            ...
            for class_value, class_info in landcover_classes.items():
                ...
                # Convert the area to hectares (1 hectare = 10,000 m²)
                areas_image[class_name] = area_image['Map'] / 10000 if area_image['Map'] else 0
            areas_collection[month] = areas_image
